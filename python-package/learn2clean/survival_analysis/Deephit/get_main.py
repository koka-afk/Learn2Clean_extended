...
def f_get_minibatch(mb_size, x, label, time, mask1, mask2):
    ...
    # Adjust mb_size if it is larger than the number of samples
    if mb_size > len(idx):
        mb_size = len(idx)
                ...
                for t, t_time in enumerate(eval_time):
                    ...
                    if eval_horizon >= num_Category:
                        ...
                        for k in range(num_Event):
                            ...
                            va_result1[k, t] = weighted_c_index(tr_time, (tr_label == k+1).astype(int), risk[:,k], va_time, (va_label == k+1).astype(int), eval_horizon)
