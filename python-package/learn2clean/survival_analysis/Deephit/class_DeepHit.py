...
##### USER-DEFINED FUNCTIONS
def log(x):
    return tf.compat.v1.log(x + _EPSILON)
...
class Model_DeepHit:
    ...
    def _build_net(self):
        tf.compat.v1.disable_eager_execution() # solves issue "tf.placeholder() is not compatible with eager execution."
        with tf.compat.v1.variable_scope(self.name):
            ...
            out = tf.compat.v1.nn.dropout(out, keep_prob=self.keep_prob)

            out = _layers.fully_connected(out, self.num_Event * self.num_Category, activation_fn=tf.nn.softmax, 
    ...
    def loss_Log_Likelihood(self):
        ...
        tmp1 = tf.compat.v1.reduce_sum(tf.compat.v1.reduce_sum(self.fc_mask1 * self.out, reduction_indices=2), reduction_indices=1, keep_dims=True)
        tmp1 = I_1 * log(tmp1)

        #for censored: log \sum P(T>t|x)
        tmp2 = tf.compat.v1.reduce_sum(tf.compat.v1.reduce_sum(self.fc_mask1 * self.out, reduction_indices=2), reduction_indices=1, keep_dims=True)
        tmp2 = (1. - I_1) * log(tmp2)
    ...
    def loss_Ranking(self):
        ...
        for e in range(self.num_Event):
            ...
            I_2 = tf.compat.v1.diag(tf.squeeze(I_2))
            ...
            diag_R = tf.reshape(tf.compat.v1.diag_part(R), [-1, 1])
            ...
            tmp_eta = tf.compat.v1.reduce_mean(T * tf.exp(-R/sigma1), reduction_indices=1, keep_dims=True)

            eta.append(tmp_eta)
        eta = tf.stack(eta, axis=1) #stack referenced on subjects
        eta = tf.compat.v1.reduce_mean(tf.reshape(eta, [-1, self.num_Event]), reduction_indices=1, keep_dims=True)
    ...
    def loss_Calibration(self):
        ...
        for e in range(self.num_Event):
            ...
            tmp_eta = tf.compat.v1.reduce_mean((r - I_2)**2, reduction_indices=1, keep_dims=True)
    ...
    def train(self, DATA, MASK, PARAMETERS, keep_prob, lr_train):
        ...
        # Reshape k_mb to match the expected shape (mb_size, 1)
        k_mb = np.reshape(k_mb, (-1, 1))
        t_mb = np.reshape(t_mb, (-1, 1))
        
        return self.sess.run([self.solver, self.LOSS_TOTAL], 
                            feed_dict={self.x: x_mb, self.k: k_mb, self.t: t_mb, 
                                        self.fc_mask1: m1_mb, self.fc_mask2: m2_mb, 
                                        self.a: alpha, self.b: beta, self.c: gamma, 
